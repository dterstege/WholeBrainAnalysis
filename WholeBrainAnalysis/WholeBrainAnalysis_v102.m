...
%   default atlas.
%
%   Requires "User_Atlas_115reg.mat", or whatever the user names it, to be in the
%   MATLAB path
%
...
%load user atlas
WB.info.useratlas = load('User_Atlas_115reg.mat'); %adjust as needed
regnum=((numel(WB.info.useratlas.User_Atlas_115reg))/3)-1; %number of regions %adjust as needed
regiondims=cell2mat(WB.info.useratlas.User_Atlas_115reg(2:end,2:3)); %identifies where regions start and end %adjust as needed
...
%       fdr_bh.m (%Benjamini, Y. & Hochberg, Y. (1995) Controlling the false discovery
%                   rate: A practical and powerful approach to multiple testing. Journal
%                   of the Royal Statistical Society, Series B (Methodological). 57(1),
%                   289-300.)
%       Brain Connectivity Toolbox (Olaf Sporns, Indiana University, 2002/2006/2008)
%
%   Outputs:
...
%   significance threshold for pariwise correlations
    WB.info.networks.threshold.pthresh = 0.005;
...
%   cutoff for binarizing matrix
    WB.info.networks.threshold.binary = 0.9;
...
for ii=1:WB.info.groupnum(1)
    ...
    neg=sum(s==-1,'omitnan')';
    pos=sum(s==1,'omitnan')';
    omit=size(pos,1)-(pos+neg);
    WB.outputs.(WB.info.groups(ii)).networks.corrbysign.negativeCors=neg;
    WB.outputs.(WB.info.groups(ii)).networks.corrbysign.positiveCors=pos;
    WB.outputs.(WB.info.groups(ii)).networks.corrbysign.omitted=omit;
    ...
    for iii=1:numsteps
        temp=cormat;
        temp(temp<tempR & temp>(-(tempR)))=NaN; %threshold
        temp(~isnan(temp))=1; %threshold
        temp(isnan(temp))=0; %threshold
